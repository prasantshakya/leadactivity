...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'employee') {
            ...
            return view('employee.index', compact( 'department', 'designation', 'employees'));
        } else {
    ...
    public function edit($id)
    {
        ...
        $employees = Employee::where('created_by', \Auth::user()->creatorId())
           ...
           // dd($employees);
        return view('employee.edit', compact('user', 'departmentData', 'employeesId', 'branches', 'employee', 'department', 'designation', 'salaryType','employees'));
    }
    ...
    if (\Auth::user()->type == 'company') {
        $validator = \Validator::make(
            ...
            [
                ...
                'exit_date'        =>  'nullable|date', // exit_date is optional
            ]
        );
        ...
        // Updating user name if provided
        ...
        if ($employee) {
            ...
            $employee->department_id = $request->department_id;  // Corrected field name
            $employee->designation_id= $request->designation_id; // Corrected field name
            $employee->team_owner    = $request->team_owner;
            $employee->dob           = \Carbon\Carbon::parse($request->dob)->format('Y-m-d');  // Using Carbon for date handling
            $employee->joining_date  = \Carbon\Carbon::parse($request->joining_date)->format('Y-m-d');  // Using Carbon
            $employee->exit_date     = $request->exit_date ? \Carbon\Carbon::parse($request->exit_date)->format('Y-m-d') : null; // Handling optional exit_date
