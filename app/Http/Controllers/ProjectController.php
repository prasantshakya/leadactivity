...
public function index()
{
    try {
        ...
        if ($user->type === 'company') {
            ...
            $projects = Project::where('created_by', $user->created_by)->get();
        } else {
            // Redirect back with an error if the user doesn't have the required type
            return redirect()->back()->with('error', __('Permission denied.'));
        }

        // Pass projects and project statuses to the view
        ...
        return redirect()->back()->with('error', __('Something went wrong while loading projects. Please try again.'));
    }
}
...
{
    try {
        // Fetch employees created by the authenticated user
        ...
        return view('project.create', compact('categories', 'projectStatus'));
        ...
        return redirect()->back()->with('error', 'Something went wrong while creating the project. Please try again.');
    }
}
...
{
    try {
        // Check if the authenticated user is of type 'company'
        if (\Auth::user()->type == 'company') {
            ...
            // Ensure a default project stage exists
            $projectStages = ProjectStage::where('created_by', \Auth::user()->creatorId())->first();
            if (empty($projectStages)) {
                return redirect()->route('project.index')->with('error', 'Please add a constant project stage.');
            }
            ...
            $project->rera_link = $request->rera_link;
            $project->unit_no = $request->unit_no;
            ...
            return redirect()->route('project.index')->with('success', __('Project successfully created.'));
        } else {
...
public function show($ids)
{
  ...
  //dd($project);
    ...
    public function edit($ids)
    {
        ...
        return view('project.edit', compact('labels','categories', 'projectStatus', 'project'));
    }
 ...
 public function update(Request $request, Project $project)
 {
    try {
        if (\Auth::user()->type == 'company') {
            ...
            $status = ($project->status != $request->status) ? 1 : 0;
        ...
        // Return error response
        return redirect()->route('project.index')->with('error', __('Something went wrong while updating the project. Please try again.'));
    }
}
